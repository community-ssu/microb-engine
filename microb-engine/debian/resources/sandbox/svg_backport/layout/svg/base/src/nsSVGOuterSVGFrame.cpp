...
nsSVGOuterSVGFrame::InitSVG()
{
  ...
  float mmPerPx = 25.4f / PresContext()->AppUnitsToDevPixels(PresContext()->AppUnitsPerInch());
...
//   }
  ...
  aDesiredSize.width = preferredRect.width;
  aDesiredSize.height = preferredRect.height;
                          ...
                          const nsRect& aDirtyRect, nsPoint aPt)
{
#if defined(DEBUG) && defined(SVG_DEBUG_PRINTING)
  {
    ...
    int fontsc;
    PresContext()->GetFontScaler(&fontsc);
    printf("font scale=%d\n",fontsc);
    printf("]\n");
  }
#endif
