...
cd $builddir && for str in `cat $TMP/trunkupdates/series`;do echo; echo Applying trunk update: $str...; echo; patch -p1 < $TMP/trunkupdates/$str; done
cd $builddir && cp -rf $TMP/trunkupdates/bin/mozilla/* .
#sh $TMP/debian/resources/sandbox/qt_port/link_it.sh $builddir $TMP/debian/resources/sandbox/qt_port
